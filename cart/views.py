...
class CartModalView(CartMixin, View):
    def get(self, request):
        ...
        return TemplateResponse(
            request,
            'cart/cart_modal.html',
            context
        )


class AddToCartView(CartMixin, View):  # работа корзины
    @transaction.atomic
    def post(self, request, slug):
        cart = self.get_cart(request)  # инициализация корзины
        product = get_object_or_404(
            Product, slug=slug)  # инициализация продукта
        ...
        if not form.is_valid():
            return JsonResponse({
                'error': 'Invalid form data',  # проверка на валидность формы
                'errors': form.errors,
            }, status=400)

        size_id = form.cleaned_data.get('size')  # проверка размера
        if size_id:
            ...
            if not product_size:
                return JsonResponse({
                    'error': 'No available sizes for this product'
                }, status=400)

        quantity = form.cleaned_data['quantity']  # проверка количества товара
        if product_size.stock < quantity:
            return JsonResponse({
                # или f'Only {product_size.stock} items available'
                'error': f'Only {product_size.stock} items in stock for size {product_size.size.name}'
            }, status=400)
        ...
        if existing_item:
            # сумма кол-ва товаров, что уже в корзине и что добавляем
    ...
    def post(self, request, item_id):
        ...
        if quantity == 0:
            ...
            if quantity > cart_item.product_size.stock:
                return JsonResponse({
                    'error': f'Only {cart_item.product_size.stock} items in stock for size {cart_item.product_size.size.name}'
                }, status=400)
...
class RemoveCartItemView(CartMixin, View):
    def post(self, request, item_id):
        ...
        try:
            ...
            return TemplateResponse(  # обновление корзины после удаления товара
                request,
                'cart/cart_modal.html',
                context
            )
